...
public class NetOutput extends Thread {
    ...
    public NetOutput(BlockingQueue<Packet> inputQueue,
                     BlockingQueue<ByteBuffer> outputQueue,
                     VpnService vpnService, Selector selector,
                     BlockingQueue<App> appCacheQueue) {
    ...
    public void run() {
        ...
        while (true) {
            ...
            Log.i(TAG,"目标ip地址:"+desAddress.getHostAddress()+"源端口:"+ sourcePort+"目的端口:"+desPort);
    ...
    private void transData(String ipAndPort, TCB tcb, Packet currentPacket, ByteBuffer dataBuffer, ByteBuffer responseBuffer) {
        ...
        //传递真实数据,添加到抓包数据中
        Log.i(TAG,"传递真实数据,添加到抓包数据中"+"包名称:"+AppUtils.getPackageNameByUid(NetKnightApp.getContext(), (int) tcb.getAppId()));
        PCapFilter.filterPacket(responseBuffer, tcb.getAppId());
        //ack码
        mOutputQueue.offer(responseBuffer);
    }
    ...
    private void initTCB(String ipAndPort, Packet referencePacket, InetAddress desAddress, int desPort, ByteBuffer responseBuffer) {
        ...
        Log.i(TAG, "passAppId=" + passAppId);
        //TODO 这里注意一下如果放开会出现抓包数据为null ,还是注释掉
       /* if (passAppId == -1) {
            //TODO 主动断开连接,拒绝访问咯
            return;
        }*/
        ...
        referencePacket.updateTCPBuffer(
                sourceBuffer,
                (byte) Packet.TCPHeader.SYN,
                referencePacket.tcpHeader.sequenceNumber,
                referencePacket.tcpHeader.acknowledgementNumber,
                0);
        ...
        TCB tcb = new TCB(
                ...
                referencePacket.tcpHeader.sequenceNumber,
                referencePacket.tcpHeader.sequenceNumber + 1,
                referencePacket.tcpHeader.acknowledgementNumber,
                referencePacket
        );
        //设置appId,方便记录流量包
